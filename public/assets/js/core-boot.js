...
class PWAEnhancer {
  ...
  showInstallUI() {
    if (this.shouldSuppressInstall()) return;
  ...
  shouldSuppressInstall() {
    if (typeof window === 'undefined') return false;
    if (window.__DISABLE_PWA_INSTALL__) return true;
    const meta = document.querySelector('meta[name="pwa-install"][content="off"]');
    if (meta) return true;
    const suppressedPaths = ['/projects', '/projects/'];
    try {
      return suppressedPaths.includes(window.location?.pathname || '');
    } catch (_) {
      return false;
    }
  }
