...
import { MatchedCTA, FollowUpSuggestions, ContextualCTAs } from '../../../src/components/islands/chat/MessageCTAs';
...
describe('MatchedCTA', () => {
	...
	it('should render CTA when query matches condition', () => {
		...
		expect(screen.getByText('Interested in hiring Blake for your next project?')).toBeInTheDocument();
	...
	it('should not render CTA when query does not match any condition', () => {
		...
		const { container } = render(<MatchedCTA message={baseMessage} messages={nonMatchingMessages} sources={[]} />);
