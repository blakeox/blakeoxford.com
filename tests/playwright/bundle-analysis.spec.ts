  ...
  test.describe('JavaScript Bundle Optimization', () => {
    ...
    test('should have optimal CSS bundle sizes', async ({ page }) => {
      ...
      if (cssBundles.length > 0) {
        ...
        // Total CSS should be under 100KB (Tailwind should be optimized)
        expect(totalCSSSize).toBeLessThan(100 * 1024);
        
        // Individual CSS files should be reasonably sized
        cssBundles.forEach(bundle => {
          expect(bundle.size).toBeLessThan(80 * 1024); // 80KB max per CSS file
        });
      ...
      if (images.length > 0) {
        ...
        images.forEach(image => {
          ...
          if (image.size > 100 * 1024 && !['webp', 'avif'].includes(image.format)) {
            console.warn(`Large image in non-optimized format: ${image.url} (${image.format}, ${Math.round(image.size / 1024)}KB)`);
          }
        });
  ...
  test.describe('Caching Strategy Analysis', () => {
    test('should implement proper caching headers', async ({ page }) => {
      ...
      if (cachingAnalysis.length > 0) {
        ...
        expect(cachingRatio).toBeGreaterThan(0.5); // At least 50% should have caching
        ...
        console.log('Caching Analysis:', {
          ...
          details: cachingAnalysis.slice(0, 5) // Show first 5 for debugging
        });
      }
    });
