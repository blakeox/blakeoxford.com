...
export class ScrollEffects {
  ...
  public setupPageTransitions(): void {
    ...
    if (document.body) {
      ...
      console.warn(
        'ScrollEffects: document.body not available, deferring progress indicator creation'
      );
      setTimeout(() => {
        if (document.body) {
          document.body.appendChild(progressIndicator);
    ...
    internalLinks.forEach((link) => {
      link.addEventListener('click', (event: MouseEvent) => {
        ...
        if (link.hasAttribute('download') || href.includes('#') || link.hasAttribute('target'))
          return;
        ...
        if (indicator) {
          indicator.style.background =
            'linear-gradient(to right, var(--color-accent) 0%, var(--color-accent-light) 50%, var(--color-accent) 100%)';
