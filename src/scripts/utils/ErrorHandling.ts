...
export class ErrorHandlingSystem {
  ...
  private handleImageError(img: HTMLImageElement): void {
    ...
    if (
      !currentSrc.includes('blake-logo-fallback.png') &&
      !img.hasAttribute('data-fallback-tried')
    ) {
    ...
    placeholder.className =
      'image-error-placeholder bg-gradient-to-br from-surface-subtle to-surface rounded-lg flex flex-col items-center justify-center text-foreground/80 text-sm border border-border';
  ...
  private showFormErrors(form: HTMLFormElement, errors: ValidationError[]): void {
    this.handleError({
      ...
      severity: 'error',
      actions: [{ label: 'Fix Errors', action: () => this.focusFirstError(form) }],
    });
  }
  ...
  private createErrorElement(error: ErrorInfo): HTMLElement {
    ...
    element.className = 'error-item bg-surface border-l-4 border-error p-4 rounded shadow-lg';
    ...
    element.className = `error-item bg-surface border-l-4 ${severityColors[error.severity]} p-4 rounded shadow-lg`;
 ...
 `;
    ...
    if (error.actions) {
      ...
      error.actions.forEach((action, index) => {
        if (buttons[index + 1]) {
          // +1 because first button is close button
  ...
  private createNotification(notification: Notification): HTMLElement {
    const element = document.createElement('div');
    element.className = 'notification bg-surface border-l-4 p-4 rounded shadow-lg';
    ...
    element.className = `notification bg-surface border-l-4 ${typeColors[notification.type]} p-4 rounded shadow-lg`;
...
if (typeof window !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      (window as Window & { errorHandlingSystem?: ErrorHandlingSystem }).errorHandlingSystem =
