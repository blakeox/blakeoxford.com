...
export default function NavBarIsland({ links, logo, currentPath }: NavBarIslandProps) {
 ...
 href={link.href}
 role="menuitem"
 className="mobile-nav-link touch-target focus-ring-interactive flex min-h-11 items-center rounded-2xl px-4 py-3 text-base font-semibold text-foreground/88 transition hover:bg-[color:var(--glass-surface-bg)] dark:hover:bg-[color:var(--glass-surface-bg-dark)]"
 aria-current={isActive(link.href) ? 'page' : undefined}
 >
 {link.label}
 </a>
