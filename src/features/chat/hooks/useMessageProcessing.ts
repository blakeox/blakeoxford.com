...
import type { ChatMessage, MutableRef, SearchFallback } from '@/lib/chat';
...
import { autoragEvents, conversionEvents } from '@/lib/analytics';
...
interface UseMessageProcessingReturn {
  ...
  assignAssistantProvenance: (
    messageId: string,
    provenance: NonNullable<ChatMessage['provenance']>
  ) => void;
      ...
      try {
        ...
        if (data.results && Array.isArray(data.results)) {
          const ranked = data.results
            ...
            .filter(
              (result: { title: string; url: string; excerpt: string; score: number }) =>
                result.score >= 0.62
            )
            .slice(0, 3);
    ...
    if (userMessages.length > 0) {
      ...
      const avgQuality =
        scored.length > 0
          ...
          : undefined;
      const totalSources = assistantMessages.reduce((sum, m) => sum + (m.sources?.length ?? 0), 0);
