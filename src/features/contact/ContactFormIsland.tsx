...
function setupTurnstile(isAudit: boolean): CleanupFn | void {
  ...
  if (isAudit) {
    container.setAttribute(
      'aria-label',
      'Human verification temporarily disabled during automated audits'
    );
  ...
  const renderWidget = () => {
    ...
    window.turnstile.render(container, {
      ...
      'before-interactive-callback': () =>
        setVerificationState('interactive', 'Complete the verification below'),
      'after-interactive-callback': () =>
        setVerificationState('loading', 'Finishing verification…'),
      'expired-callback': () =>
...
function setupContactForm(): CleanupFn | void {
  ...
  const statusElement = document.querySelector<HTMLElement>(
    FORM_VALIDATION_CONFIG.statusElementSelector ?? ''
  );
...
export default function ContactFormIsland() {
  useEffect(() => {
    ...
    const cleanupFns: Array<CleanupFn | void> = [setupContactForm()];
