...
export function createNetworkError(
  ...
  const code = options.status ? getErrorCodeFromStatus(options.status) : ErrorCodes.NETWORK_UNKNOWN;
...
function getChatErrorMessage(code: ErrorCode): string {
  const messages: Record<string, string> = {
    [ErrorCodes.CHAT_CONNECTION_FAILED]:
      'Unable to connect. The AI assistant is temporarily unavailable.',
    [ErrorCodes.CHAT_MESSAGE_FAILED]: "Your message couldn't be sent. Please try again.",
    [ErrorCodes.CHAT_TIMEOUT]:
      'The AI is thinking longer than usual. Please try a simpler question.',
    [ErrorCodes.AI_INFERENCE_ERROR]:
      'I encountered an error processing your request. Please rephrase and try again.',
    [ErrorCodes.AI_RATE_LIMITED]:
      "You've reached the message limit. Please wait a moment before asking more questions.",
  };
...
// ─── Error Categorization (query / retry helpers) ─────────────────────────────
