  ...
  },
  {
    ...
    props: [
      { name: 'sources', type: 'Source[]', required: true, description: 'Array of source objects' },
      {
        ...
        type: 'string',
        required: true,
        description: 'Message ID for stable citation keys',
      },
      {
        name: 'siteHostname',
        type: 'string',
        required: true,
        description: 'Site hostname used to detect external citation URLs',
      },
      {
        name: 'sourceRefs',
        type: 'RefObject<HTMLAnchorElement[]>',
        required: false,
        description: 'Optional refs for keyboard focus on citation links',
      },
      {
        name: 'onOpenSource',
        type: '(url: string) => void',
        required: false,
        description: 'Optional callback when a citation link is activated',
      },
    ],
    examples: [
      {
        title: 'Citation links',
        code: '<MessageSources sources={msg.sources} messageId={msg.id} siteHostname="blakeoxford.com" />',
      },
    ],
    accessibility: [
    ...
    performance: ['React.memo', 'Ranks and filters to at most two display sources'],
    tags: ['react', 'chat', 'citations', 'sources'],
  },
  {
    name: 'MessageActions',
    category: 'Islands',
