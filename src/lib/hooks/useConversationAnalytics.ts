...
import { INITIAL_ASSISTANT_MESSAGE } from '../chat';
...
export function useConversationAnalytics(
  ...
  const { messages, interimTranscript = '', inputValue = '', composerFocused = false } = options;
   ...
   */
  const canStartNewChat = useMemo(() => messages.length > 1, [messages.length]);
