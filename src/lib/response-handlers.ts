...
export async function finalizeMessageQuality(
	...
	try {
		...
		logger.error('LLM evaluation failed, using heuristic score:', error);
 ...
 */
export function getRelevanceExplanation(score: number): { text: string; color: string; icon: string } {
	if (score >= 90) {
		return {
