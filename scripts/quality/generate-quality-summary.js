...
function sparkline(values) {
  ...
  return values
    .map(
      (v) =>
        blocks[Math.min(blocks.length - 1, Math.floor(((v - min) / range) * (blocks.length - 1)))]
    )
    .join('');
}
...
function summarizePerformance() {
  ...
  if (fs.existsSync(perfHistoryPath)) {
    try {
      history = loadJSON(perfHistoryPath);
    } catch {
      history = [];
    }
  }
  ...
  for (const [route, metrics] of Object.entries(baseline.routes || {})) {
    ...
    const pctSlope = loadSeries.length
      ? (slope / (loadSeries.reduce((a, b) => a + b, 0) / loadSeries.length)) * 100
      : 0;
    ...
    const note =
      classification !== 'stable' && classification !== 'noise' ? ` (${classification})` : '';
...
function summarizeFlakiness() {
  ...
  try {
    ...
    const avgRetriesPerRun =
      active.reduce((a, t) => a + (t.totalRetries || 0), 0) /
      (active.reduce((a, t) => a + (t.runs || 1), 0) || 1);
    const intensitySeries = active
      .map((t) => (t.totalRetries || 0) / (t.runs || 1))
      .sort((a, b) => b - a)
      .slice(0, 12)
      .reverse();
    const { slope, direction } = calcTrend(intensitySeries);
    const pctSlope = intensitySeries.length
      ? (slope / (intensitySeries.reduce((a, b) => a + b, 0) / intensitySeries.length || 1)) * 100
      : 0;
    let classification = 'stable';
    if (direction === 'regressing') {
    ...
    const topFlaky = active
      .filter((t) => t.flaky)
      .sort((a, b) => (b.totalRetries || 0) - (a.totalRetries || 0))
      .slice(0, 5)
      .map((t) => `  - ${t.id} (retries:${t.totalRetries || 0}/runs:${t.runs || 1})`)
      .join('\n');
    const lines = [
      '### Flakiness',
      '',
      `- Tracked tests (excluding quarantine): ${totalTests}`,
      `- Currently flaky (last run, excluding quarantine): ${flakyNow}`,
      `- Currently failed (last run, excluding quarantine): ${failedNow}`,
      `- Avg retries per test-run (cumulative, excluding quarantine): ${avgRetriesPerRun.toFixed(2)}`,
      `- Retry intensity trend: ${isFinite(pctSlope) ? pctSlope.toFixed(2) + '%' : 'n/a'} ${emoji} (${classification}) ${series}`,
    ];
    if (quarantined.length) {
      const qList = quarantined
        .slice(0, 10)
        .map((t) => `  - ${t.id} (retries:${t.totalRetries || 0}, failures:${t.failures || 0})`)
        .join('\n');
...
function main() {
  ...
  if (!diffs.length)
    parts.push('### API Diff Reports', '', '_No API baseline changes detected._', '');
  ...
  if (fs.existsSync(searchRelevancePath)) {
    try {
      ...
      const topRate =
        typeof sr.topNPassRate === 'number' ? sr.topNPassRate.toFixed(1) : sr.topNPassRate;
      const strictRate =
        typeof sr.strictPassRate === 'number' ? sr.strictPassRate.toFixed(1) : sr.strictPassRate;
      parts.push(
        '### Search Relevance',
        '',
        `- Strict Pass: ${strictRate}%`,
        `- Top-${sr.topN} Pass: ${topRate}%`,
        sr.minTopNRequired
          ? `- Gate: require >= ${sr.minTopNRequired}% => ${sr.gatePassed ? 'PASSED' : 'FAILED'}`
          : '',
        ''
      );
    } catch {
    ...
    try {
      ...
      if (hist.length) {
        ...
        parts.push(
          '### Accessibility Trend',
          '',
          `- Recent totals: ${perRunTotals.join(', ')}`,
          `- Trend slope: ${slope.toFixed(2)} (${direction}) ${spark}`,
          ''
        );
      }
    } catch {
  ...
  if (fs.existsSync(longTasksReportPath)) {
    try {
      ...
      const summaryLine = lt
        .map((r) => `${r.route} max:${r.max.toFixed ? r.max.toFixed(1) : r.max} over50:${r.over50}`)
        .join(' | ');
      ...
      if (tox.length) {
        parts.push(
          ...
          tox
            .map(
              (t) =>
                `- ${t.id} score=${t.score.toFixed(3)} retries=${t.avgRetries.toFixed ? t.avgRetries.toFixed(2) : t.avgRetries} failRate=${t.failRate} avgDuration=${t.avgDuration}ms`
            )
            .join('\n'),
          ''
        );
