...
function semverMajor(v){ const m = v.match(/(\d+)\.(\d+)\.(\d+)/); return m?parseInt(m[1],10):null; }
...
(async () => {
  ...
  const lines = results.map(r => `${r.name}@${r.declared} -> latest ${r.latest}${r.majorLag!=null?` (lag ${r.majorLag} major)`:''}${r.ageDays!=null?` age ${r.ageDays}d`:''}`);
