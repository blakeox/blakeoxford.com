...
async function main() {
  ...
  console.log(`[auto-close] Stability check over last ${days} days: ${stable ? 'stable (all zero)' : 'not stable'}`);
  if (!stable) return;
  ...
  for (const issue of issues) {
    try {
      execSync(`gh issue close ${issue.number} --comment "Auto-closed: ${days} consecutive days with zero borderline contrast items. ✅"`, { stdio: 'inherit' });
    } catch (e) {
