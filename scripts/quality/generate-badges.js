...
try {
  ...
  if (fhRaw) {
    ...
    if (fh && Array.isArray(fh.runs) && fh.runs.length) {
      ...
      if (typeof latest.passRate === 'number')
        passRateLabel = (latest.passRate * 100).toFixed(1) + '%';
    }
  }
} catch {
  /* ignore parse errors */
  ...
  if (a11yRaw) {
    ...
    if (last) {
      if (last.totals && typeof last.totals.count === 'number') {
        ...
        total = last.pages.reduce(
          (acc, p) =>
            acc + (typeof p.violations === 'number' && p.violations > 0 ? p.violations : 0),
          0
        );
      }
    }
