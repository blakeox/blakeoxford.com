...
for (const file of Object.keys(baseline.files ?? {})) {
	if (!(file in current.files)) {
		console.log(`[blog-mdx-dark-baseline] ${file} no longer has dark: utilities (baseline can be ratcheted)`);
	}
}
...
if (errors.length) {
	...
	console.error('Then: node scripts/quality/check-blog-mdx-dark-baseline.js --update (if intentional)');
	process.exit(1);
}
