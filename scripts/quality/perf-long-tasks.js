...
(async () => {
  ...
  await page.goto(base + route, { waitUntil: 'domcontentloaded' }).catch(()=>{});
    ...
    await page.addInitScript(() => {
      ...
      new PerformanceObserver(list => {
        // eslint-disable-next-line no-undef
        list.getEntries().forEach(e => window.__longTasks.push({ name: e.name, start: e.startTime, duration: e.duration }));
  ...
  const tasks = await page.evaluate(() => (window.__longTasks || []));
    ...
    report.push({ route, total: tasks.length, over50: over50.length, max: tasks.reduce((m,t)=> Math.max(m,t.duration),0) || 0 });
  }
  await browser.close();
  fs.writeFileSync('long-tasks-report.json', JSON.stringify(report,null,2));
  console.log('[longtasks] ' + report.map(r=>`${r.route}:>${r.over50} max=${r.max.toFixed(1)}`).join(' | '));
})();
