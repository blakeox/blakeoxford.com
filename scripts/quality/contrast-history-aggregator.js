...
process.stdin.on('end', () => {
  ...
  const totals = Object.values(todayEntry.routes).reduce((acc, r) => {
  ...
  const slopeThreshold = process.env.CONTRAST_SLOPE_ALERT ? parseFloat(process.env.CONTRAST_SLOPE_ALERT) : null;
  ...
  console.log(`Updated contrast history for ${payloads.length} routes. Total sampled today: ${totals.sampled}. Borderline=${totals.borderline} (threshold=${alertThreshold}). rolling7=${todayEntry.metrics.rolling7Borderline} slope7=${slopeVal} slopeAlertThreshold=${slopeThreshold ?? 'n/a'}`);
  if (countAlert) {
    console.log(`[contrast-alert] Borderline count ${totals.borderline} exceeds threshold ${alertThreshold}.`);
    // Provide last 3-day markdown snippet for issue context
    const recent = history.slice(-3);
    let md = '| Date | Borderline | Sampled | Rolling7 | Slope7 | StdDev7 |\n|------|-----------:|--------:|---------:|-------:|---------:|\n';
    ...
    console.log(`[contrast-alert] Borderline slope ${slopeVal} exceeds slope threshold ${slopeThreshold}.`);
    const recent = history.slice(-3);
    let md = '| Date | Borderline | Sampled | Rolling7 | Slope7 | StdDev7 |\n|------|-----------:|--------:|---------:|-------:|---------:|\n';
    for (const e of recent) {
      md += `| ${e.date} | ${e.summary?.borderline ?? 0} | ${e.summary?.sampled ?? 0} | ${e.metrics?.rolling7Borderline ?? '—'} | ${e.metrics?.slope7Borderline ?? '—'} | ${e.metrics?.stddev7Borderline ?? '—'} |\n`;
