...
function fail(msg) {
  console.error(`❌ Flakiness Gate: ${msg}`);
  process.exit(1);
}
function info(msg) {
  console.log(`ℹ️  Flakiness Gate: ${msg}`);
}
...
let parsed;
try {
  parsed = JSON.parse(fs.readFileSync(historyPathInUse, 'utf-8'));
} catch {
  fail('Unable to parse flakiness-history.json');
}
...
if (Array.isArray(parsed)) {
  if (!parsed.length) {
    info('Empty history (legacy array); skipping');
    process.exit(0);
  }
  ...
  if (!parsed.runs.length) {
    info('Empty history (run-level); skipping');
    process.exit(0);
  }
...
const maxFlaky = process.env.FLAKINESS_MAX_CURRENT_FLAKY
  ? parseInt(process.env.FLAKINESS_MAX_CURRENT_FLAKY, 10)
  : null;
const maxIntensity = process.env.FLAKINESS_MAX_RETRY_INTENSITY
  ? parseFloat(process.env.FLAKINESS_MAX_RETRY_INTENSITY)
  : null;
const minReliability = process.env.FLAKINESS_MIN_PASS_RATE
  ? parseFloat(process.env.FLAKINESS_MIN_PASS_RATE)
  : null; // expect 0-1 range

let violated = false;
if (maxFlaky !== null && currentFlaky > maxFlaky) {
  violated = true;
  fail(`Currently flaky tests (${currentFlaky}) exceed limit (${maxFlaky})`);
}
if (maxIntensity !== null && avgRetryIntensity > maxIntensity) {
  violated = true;
  fail(`Avg retry intensity (${avgRetryIntensity.toFixed(4)}) exceeds limit (${maxIntensity})`);
}
if (minReliability !== null && currentPassRate != null && currentPassRate < minReliability) {
  violated = true;
  fail(
    `Pass rate (${(currentPassRate * 100).toFixed(2)}%) below minimum (${(minReliability * 100).toFixed(2)}%)`
  );
}

if (!violated)
  info(
    `PASS (flaky:${currentFlaky}${maxFlaky !== null ? `<=${maxFlaky}` : ''}, intensity:${avgRetryIntensity.toFixed(4)}${maxIntensity !== null ? `<=${maxIntensity}` : ''}, passRate:${currentPassRate != null ? (currentPassRate * 100).toFixed(2) + '%' : ''}${minReliability !== null ? `>=${(minReliability * 100).toFixed(2)}%` : ''})`
  );
process.exit(0);
